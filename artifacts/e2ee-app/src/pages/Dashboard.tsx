...
export default function Dashboard() {
  ...
  const handleSendMessage = async () => {
    if (!messageInput.trim() || !selectedPartnerId || !selectedPartnerPublicKey || !privateKeyBytes || !user) return;
          ...
          {!selectedPartnerId ? (
            <div className="flex-1 flex flex-col items-center justify-center text-center p-8 space-y-4">
              ...
              <div className="text-muted-foreground text-sm max-w-xs">
                Select a conversation or click + to start a new encrypted channel.
              </div>
              <div className="text-xs text-muted-foreground font-mono bg-card border border-card-border px-3 py-2">
                All messages encrypted with AES-GCM + ECDH before leaving your device
              </div>
            </div>
          ) : (
            <>
